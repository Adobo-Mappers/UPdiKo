...
/**
 * Gets driving/walking route coordinates using OSRM (free, no backend needed).
 * Falls back to empty array on failure so the UI degrades gracefully.
 *
 * @param {number} startLat
 * @param {number} startLng
 * @param {number} endLat
 * @param {number} endLng
 * @returns {Promise<number[][]>} Array of [lat, lng] coordinate pairs for the polyline
 */
export const getRoute = async (startLat, startLng, endLat, endLng) => {
  try {
    ...
    // Convert [lng, lat] pairs from GeoJSON to [lat, lng] for Leaflet
